...
- **バックアップ＆リセット** — チャンネルのシートをバックアップ付きでリセット可能
...
| `@Log Bot reset` | そのチャンネルのシートをバックアップ＆リセット |
...
| `@Log Bot share` | その場で手動同期 |

自動解除の対象は **bot が付与した閲覧者のみ**です。オーナーや、手で追加した編集者・
コメント権限には触れません。
...
| 日時 | ユーザー名 | メッセージ | 添付ファイル | パーマリンク | メッセージTS | スレッドTS |
|------|-----------|-----------|-------------|-------------|-------------|-----------|
...
> チャンネル名の列はありません。1チャンネル=1スプレッドシートなので、
> 全行に同じ値が並ぶだけだからです。表示名の列も、ユーザー名と重複するため外しています。
...
| 行の高さ | 全データ行 84px 固定（約4行分） |
| 列幅 | 合計 1110px（ノートPCの画面幅に収まる想定） |
| TS列 | グレー小文字（メタデータとして控えめ表示） |

> 行の高さは **固定値**です。Google スプレッドシートに「最大の高さ」という設定はないため、
> 84px を超える長文は表示が途中で切れます。**データは全文保持されており**、
> セルを選べば数式バーに全文が出ます。行を広げれば全体も表示できます。
> 常に全文を表示したい場合は `google_sheets.py` の `DATA_ROW_HEIGHT` を大きくしてください。
